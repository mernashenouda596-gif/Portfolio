...
export default function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    const ps:P[] = Array.from({length:70},()=>({x:Math.random()*cv.width,y:Math.random()*cv.height,vx:(Math.random()-.5)*.35,vy:(Math.random()-.5)*.35,size:Math.random()*2.2+.4,opacity:Math.random()*.45+.1,ci:Math.floor(Math.random()*PALETTE.length)}));
